...
struct ImmichAssetCollectionView: View {
    let title: String
    let albumIDs: [String]
    let tagIDs: [String]

    @EnvironmentObject private var appModel: ImmichAppModel
    @StateObject private var viewModel: ImmichAssetCollectionViewModel
    @State private var selectedAssetID: String?
    @State private var gridItemWidth: CGFloat = 110

    private let gridSpacing: CGFloat = 2

    init(title: String, albumIDs: [String] = [], tagIDs: [String] = []) {
        self.title = title
        self.albumIDs = albumIDs
        self.tagIDs = tagIDs
        _viewModel = StateObject(wrappedValue: ImmichAssetCollectionViewModel(albumIDs: albumIDs, tagIDs: tagIDs))
    }
                    ...
                    LazyVStack(alignment: .leading, spacing: 0) {
                        ...
                        if viewModel.isLoading && viewModel.assets.isEmpty {
                            ...
                            ContentUnavailableView(
                                ...
                                description: Text("Immich returned an empty result set.")
                            )
    ...
    private var taskIdentity: String {
        "\(appModel.reloadToken)|\(appModel.configuration?.baseURL.absoluteString ?? "nil")|\(appModel.configuration?.apiKey ?? "nil")|\(albumIDs.joined(separator: ","))|\(tagIDs.joined(separator: ","))"
    }
